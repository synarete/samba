			     ...
			     DATA_BLOB *data)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG(1, ("Invalid MSG_DEBUG from pid %u to pid %u: %s\n",
			  (unsigned int)procid_to_pid(&src),
			  (unsigned int)getpid(),
			  ndr_errstr(ndr_err)));
		goto out;
	...
	DEBUG(3, ("INFO: Remote set of debug to `%s'  (pid %u from pid %u)\n",
		  msg.info.info1.debug_string, (unsigned int)getpid(),
		  (unsigned int)procid_to_pid(&src)));

	debug_parse_levels(msg.info.info1.debug_string);
out:
	TALLOC_FREE(frame);
}
				  ...
				  DATA_BLOB *data)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG(0, ("Invalid MSG_REQ_DEBUGLEVEL from PID %s: %s\n",
			  server_id_str_buf(src, &tmp),
			  ndr_errstr(ndr_err)));
		goto out;
	}
	...
	ndr_err = messaging_debuglevel_push_v1(frame, &reply, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
