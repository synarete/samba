...
static void ping_message_v1(struct messaging_context *msg_ctx,
			    void *private_data,
			    uint32_t msg_type,
			    struct server_id src,
			    DATA_BLOB *data)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG(1, ("Invalid MSG_PING from PID %s: %s\n",
	...
	ndr_err = messaging_pong_push_v1(frame, &pong, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
	messaging_send(msg_ctx, src, MSG_PONG_V1, &blob);
out:
	TALLOC_FREE(frame);
}
					...
					struct messaging_context **pmsg_ctx)
{
	...
	messaging_register(ctx, NULL, MSG_PING_V1, ping_message_v1);
