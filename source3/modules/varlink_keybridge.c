		       ...
		       VarlinkObject *obj)
{
	...
	if (vret < 0) {
		...
		result->data = talloc_strdup(
			result, "varlink_object_to_json failed");
		return;
	}
			   ...
			   struct varlink_keybridge_result **resp)
{
	...
	if (vret < 0) {
		DBG_ERR_VLKB("varlink_object_set_string '" KB_F_NAME "' failed", -vret);
		goto done;
	}
	vret = varlink_object_set_string(params, KB_F_SCOPE, kbc->scope);
	if (vret < 0) {
		DBG_ERR_VLKB("varlink_object_set_string '" KB_F_SCOPE "' failed", -vret);
		goto done;
	}
	vret = varlink_object_set_string(
		params, KB_F_KIND, vlkb_kind_string(kbc->kind));
	if (vret < 0) {
		DBG_ERR_VLKB("varlink_object_set_string '" KB_F_KIND "' failed", -vret);
		goto done;
	}
