...
static bool ratelimiter_has_iops_overflow(const struct ratelimiter *rl)
{
	return (rl->iops_limit > 0) && (rl->iops_curr > rl->iops_limit);
}

static bool ratelimiter_has_bytes_overflow(const struct ratelimiter *rl)
{
	return (rl->bytes_limit > 0) && (rl->bytes_curr > rl->bytes_limit);
}
...
static uint32_t ratelimiter_delay(const struct ratelimiter *rl)
{
	...
	if (ratelimiter_has_iops_overflow(rl) ||
	    ratelimiter_has_bytes_overflow(rl))
	{
		msec_delay = rl->msec_delay;
	}
	return msec_delay;
}
...
static void aio_qos_setup_ratelimiters(struct aio_qos_config *config)
{
	...
	ratelimiter_init(&config->wr_ratelimiter, iops, bytes, delay, false);
...
static uint32_t aio_qos_post(struct aio_qos_state *state)
{
	...
	if ((state->rl != NULL) && (state->result > 0)) {
		const size_t bytes = (size_t)state->result;

		ratelimiter_update(state->rl, bytes);
		delay_msec = ratelimiter_delay(state->rl);
	}
	return delay_msec;
}
