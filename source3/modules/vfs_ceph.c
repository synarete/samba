...
{
	const char *conf_file = lp_parm_const_string(snum,
						     "ceph",
						     "config_file",
						     ".");
	const char *user_id = lp_parm_const_string(snum, "ceph", "user_id", "");
	const char *fsname = lp_parm_const_string(snum,
						  "ceph",
						  "filesystem",
						  "");
	return talloc_asprintf(
		mem_ctx, "(%s/%s/%s)", conf_file, user_id, fsname);
}
...
static struct ceph_mount_info *cephmount_mount_fs(const int snum)
{
	...
	const char *conf_file = lp_parm_const_string(snum,
						     ...
						     "config_file",
						     NULL);
	const char *user_id = lp_parm_const_string(snum,
						   "ceph",
						   "user_id",
						   NULL);
	const char *fsname = lp_parm_const_string(snum,
				   ...
				   uint64_t *dsize)
{
	...
	if (!(ret = ceph_statfs(handle->data,
				smb_fname->base_name,
				&statvfs_buf)))
	{
		/*
...
static int cephwrap_mkdirat_path(struct vfs_handle_struct *handle,
				 files_struct *dirfsp,
				 const struct smb_filename *smb_fname,
				 mode_t mode)
{
...
static int cephwrap_mkdirat_fd(struct vfs_handle_struct *handle,
			       files_struct *dirfsp,
			       const struct smb_filename *smb_fname,
			       mode_t mode)
{
	int dfd = fsp_get_pathref_fd(dirfsp);
	int result;
	...
	result = ceph_mkdirat(handle->data, dfd, smb_fname->base_name, mode);
	DBG_DEBUG("[CEPH] mkdirat(...) = %d\n", result);
	return WRAP_RETURN(result);
}
#endif

static bool fsp_has_non_pathref_fd(const files_struct *fsp)
{
	bool status;

#ifdef HAVE_LIBCEPHFS_NEW
	status = !fsp->fsp_flags.is_pathref && !(fsp_get_pathref_fd(fsp) < 0);
#else
	status = false;
#endif
	return status;
			    ...
			    mode_t mode)
{
	...
	if (!fsp_has_non_pathref_fd(dirfsp)) {
		result = cephwrap_mkdirat_path(handle, dirfsp, smb_fname, mode);
	} else {
#ifdef HAVE_LIBCEPHFS_NEW
		result = cephwrap_mkdirat_fd(handle, dirfsp, smb_fname, mode);
#endif
	}
	return result;
}
...
/* File operations */
static int cephwrap_openat_path(struct vfs_handle_struct *handle,
...
#ifdef HAVE_LIBCEPHFS_NEW
static int cephwrap_openat_fd(struct vfs_handle_struct *handle,
			      const struct files_struct *dirfsp,
			      const struct smb_filename *smb_fname,
			      files_struct *fsp,
			      const struct vfs_open_how *how)
			   ...
			   const struct vfs_open_how *how)
{
	...
	if (!fsp_has_non_pathref_fd(dirfsp)) {
		result = cephwrap_openat_path(
			handle, dirfsp, smb_fname, fsp, how);
	} else {
#ifdef HAVE_LIBCEPHFS_NEW
		result = cephwrap_openat_fd(
			handle, dirfsp, smb_fname, fsp, how);
#endif
	}
	return result;
...
static ssize_t cephwrap_pread_recv(struct tevent_req *req,
				   struct vfs_aio_state *vfs_aio_state)
{
	struct cephwrap_pread_state *state = tevent_req_data(
		req, struct cephwrap_pread_state);
...
static ssize_t cephwrap_pwrite_recv(struct tevent_req *req,
				    struct vfs_aio_state *vfs_aio_state)
{
	struct cephwrap_pwrite_state *state = tevent_req_data(
		req, struct cephwrap_pwrite_state);
					 ...
					 const struct ceph_statx *stx)
{
	...
	if ((stx->stx_mask & SAMBA_STATX_ATTR_MASK) != SAMBA_STATX_ATTR_MASK) {
		DBG_WARNING(
			"%s: stx->stx_mask is incorrect (wanted %x, got %x)\n",
			__func__,
			  ...
			  SMB_STRUCT_STAT *sbuf)
{
	...
	result = ceph_fstatx(handle->data, fd, &stx, SAMBA_STATX_ATTR_MASK, 0);
			    ...
			    struct smb_file_time *ft)
{
	...
	if (!fsp->fsp_flags.is_pathref) {
		 ...
		 */
		result = ceph_setattrx(
			handle->data, fsp->fsp_name->base_name, &stx, mask, 0);
	}
...
static int cephwrap_unlinkat_path(struct vfs_handle_struct *handle,
				  struct files_struct *dirfsp,
				  const struct smb_filename *smb_fname,
				  int flags)
{
...
#ifdef HAVE_LIBCEPHFS_NEW
static int cephwrap_unlinkat_fd(struct vfs_handle_struct *handle,
				struct files_struct *dirfsp,
				const struct smb_filename *smb_fname,
				int flags)
{
			     ...
			     int flags)
{
	...
	if (!fsp_has_non_pathref_fd(dirfsp) || (flags & AT_REMOVEDIR)) {
		result = cephwrap_unlinkat_path(handle,
						dirfsp,
						smb_fname,
						flags);
	} else {
#ifdef HAVE_LIBCEPHFS_NEW
		result = cephwrap_unlinkat_fd(handle, dirfsp, smb_fname, flags);
#endif
	}
	return result;
}
...
static int cephwrap_symlinkat_path(struct vfs_handle_struct *handle,
...
#ifdef HAVE_LIBCEPHFS_NEW
static int cephwrap_symlinkat_fd(struct vfs_handle_struct *handle,
				 const struct smb_filename *link_target,
				 struct files_struct *dirfsp,
				 const struct smb_filename *new_smb_fname)
{
			      ...
			      const struct smb_filename *new_smb_fname)
{
	...
	if (!fsp_has_non_pathref_fd(dirfsp)) {
		result = cephwrap_symlinkat_path(handle,
						 link_target,
						 dirfsp,
						 new_smb_fname);
	} else {
#ifdef HAVE_LIBCEPHFS_NEW
		result = cephwrap_symlinkat_fd(handle,
					       link_target,
					       dirfsp,
					       new_smb_fname);
#endif
...
static int cephwrap_readlinkat_path(struct vfs_handle_struct *handle,
				    const struct files_struct *dirfsp,
				    const struct smb_filename *smb_fname,
				    char *buf,
				    size_t bufsiz)
...
#ifdef HAVE_LIBCEPHFS_NEW
static int cephwrap_readlinkat_fd(struct vfs_handle_struct *handle,
				  const struct files_struct *dirfsp,
				  const struct smb_filename *smb_fname,
				  char *buf,
				  size_t bufsiz)
			       ...
			       size_t bufsiz)
{
	...
	if (!fsp_has_non_pathref_fd(dirfsp)) {
		result = cephwrap_readlinkat_path(
			handle, dirfsp, smb_fname, buf, bufsiz);
	} else {
#ifdef HAVE_LIBCEPHFS_NEW
		result = cephwrap_readlinkat_fd(
			handle, dirfsp, smb_fname, buf, bufsiz);
#endif
	}
	return result;
 ...
 */
static struct smb_filename *cephwrap_realpath(
	struct vfs_handle_struct *handle,
	TALLOC_CTX *ctx,
	const struct smb_filename *smb_fname)
{
...
static NTSTATUS cephwrap_get_real_filename_at(struct vfs_handle_struct *handle,
					      struct files_struct *dirfsp,
					      const char *name,
					      TALLOC_CTX *mem_ctx,
					      char **found_name)
				  ...
				  size_t size)
{
	...
	DBG_DEBUG("[CEPH] fgetxattr(%p, %d, %s, %p, %llu)\n",
		  handle,
		  fd,
		  name,
		  value,
		  llu(size));
	ret = ceph_fgetxattr(handle->data, fd, name, value, size);
			       ...
			       struct files_struct *fsp)
{
	...
	DBG_DEBUG(
		"[CEPH] cephwrap_aio_force(%p, %p) = false (errno = ENOTSUP)\n",
		handle,
		fsp);
	errno = ENOTSUP;
	return false;
					   ...
					   size_t referral_count)
{
	...
	msdfs_link = msdfs_link_string(frame, reflist, referral_count);
	if (msdfs_link == NULL) {
		goto out;
	}

	ret = ceph_symlink(handle->data, msdfs_link, full_fname->base_name);
	if (ret == 0) {
		status = NT_STATUS_OK;
	} else {
					 ...
					 size_t *preferral_count)
{
	...
	DBG_INFO("%s -> %s\n", full_fname->base_name, link_target);
...
{
	return smb_register_vfs(SMB_VFS_INTERFACE_VERSION, "ceph", &ceph_fns);
}
