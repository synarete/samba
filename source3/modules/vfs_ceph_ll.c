...
static int vfs_ceph_unmount(struct vfs_handle_struct *handle)
{
	...
	CEPH_DBG("snum=%d", snum_of(handle));
			      ...
			      off_t off)
{
	...
	CEPH_DBG("ino=%ld fd=%d off=%jd len=%zu",
		 cfh->iref.ino,
		 cfh->fd,
		 (intmax_t)off,
		 len);
			       ...
			       off_t off)
{
	...
	CEPH_DBG("ino=%ld fd=%d len=%zu off=%jd",
		 cfh->iref.ino,
		 cfh->fd,
		 len,
		 (intmax_t)off);
			    ...
			    int whence)
{
	...
	CEPH_DBG("ino=%ld fd=%d offset=%jd whence=%d",
		 cfh->iref.ino,
		 cfh->fd,
		 (intmax_t)offset,
		 whence);
			      ...
			      off_t len)
{
	...
	CEPH_DBG("ino=%ld fd=%d mode=%x off=%jd len=%jd",
		 cfh->iref.ino,
		 cfh->fd,
		 mode,
		 (intmax_t)off,
			    ...
			    SMB_DEV_T dev)
{
	...
	CEPH_DBG("dino=%ld name=%s mode=%o dev=%ld",
		 diref.ino,
		 smb_fname->base_name,
		 mode,
		 (long)dev);
