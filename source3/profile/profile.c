			       ...
			       DATA_BLOB *data)
{
	...
	if (msg.info.info1.level > 3) {
		DEBUG(0, ("got invalid MSG_PROFILE_V1 level: %u\n",
			  msg.info.info1.level));
		TALLOC_FREE(frame);
		return;
	}

	set_profile_level(msg.info.info1.level, &src);
	TALLOC_FREE(frame);
}
				  ...
				  DATA_BLOB *data)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DEBUG(0, ("Invalid MSG_REQ_PROFILELEVEL from PID"
			  " %u: %s\n",
			  (unsigned int)procid_to_pid(&src),
			  ndr_errstr(ndr_err)));
		goto out;
