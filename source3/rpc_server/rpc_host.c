...
static NTSTATUS rpc_host_exit_worker_v1(struct messaging_context *msg_ctx,
					pid_t pid)
{
	...
	void *private_data)
{
	...
	for (i=0; i<num_workers; i++) {
		...
		if (messaging_has_cluster_level_upgraded(
			    server->host->msg_ctx)) {
			status = rpc_host_exit_worker_v1(
				server->host->msg_ctx, w->pid);
		} else {
