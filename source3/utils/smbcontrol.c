			       ...
			       DATA_BLOB *data)
{
	...
	ndr_err = messaging_profilelevel_v1_pull(frame, data, &reply);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		fprintf(stderr,
			       ...
			       const struct server_id pid)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_req_profilelevel_v1_push(frame, &msg, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
			     ...
			     const char *sharename)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_smb_force_tdis_v1_push(frame,
						   &msg,
						   sharename,
						   &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
				     ...
				     const char *sharename)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_smb_force_tdis_denied_v1_push(frame,
							  &msg,
							  sharename,
							  &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
				    ...
				    const char *client_ip)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_smb_kill_client_ip_v1_push(frame,
						       &msg,
						       client_ip,
						       &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
			       ...
			       const struct server_id pid)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_reload_tls_certificates_v1_push(frame,
							    &msg,
							    &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
