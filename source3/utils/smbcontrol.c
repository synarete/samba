...
static bool do_ping_v1(struct tevent_context *ev_ctx,
		       struct messaging_context *msg_ctx,
		       const struct server_id pid)
{
	TALLOC_CTX *frame = NULL;
		    ...
		    const char **argv)
{
	...
	if (messaging_has_cluster_level_upgraded(msg_ctx)) {
		return do_ping_v1(ev_ctx, msg_ctx, pid);
	}
			  ...
			  const int argc, const char **argv)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_force_tdis_push(frame, &msg, argv[1], &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
	...
	const int argc, const char **argv)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_force_tdis_denied_push(frame,
						   &msg,
						   argv[1],
						   &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
				 ...
				 const int argc, const char **argv)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_kill_client_ip_push(frame, &msg, argv[1], &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
...
static void print_uint32_cb(struct messaging_context *msg,
			    void *private_data,
			    uint32_t msg_type,
			    struct server_id pid,
			    DATA_BLOB *data)
...
static void print_num_children_v1_cb(struct messaging_context *msg,
				     void *private_data,
				     uint32_t msg_type,
				     struct server_id pid,
				     DATA_BLOB *data)
			    ...
			    const int argc, const char **argv)
{
	...
	messaging_register(msg_ctx,
			   NULL,
			   MSG_SMB_NUM_CHILDREN_V1,
			   print_num_children_v1_cb);
...
out:
	...
	messaging_deregister(msg_ctx, MSG_SMB_NUM_CHILDREN_V1, NULL);
	TALLOC_FREE(frame);
	return ok;
}
