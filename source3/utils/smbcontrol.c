		     ...
		     const int argc, const char **argv)
{
	...
	ndr_err = messaging_debug_push(frame, &msg, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}

	ok = send_message(msg_ctx, pid, MSG_DEBUG, blob.data, blob.length);
out:
	TALLOC_FREE(frame);
	return ok;
}
		     ...
		     const int argc, const char **argv)
{
	...
	if (strcmp(argv[1], "delete") == 0) {
		...
		ok = send_message(
			msg_ctx, pid, ID_CACHE_KILL, blob.data, blob.length);
		     ...
		     const int argc, const char **argv)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_smb_sleep_push(frame, &msg, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}

	ok = send_message(msg_ctx, pid, MSG_SMB_SLEEP, blob.data, blob.length);
out:
	TALLOC_FREE(frame);
	return ok;
#endif /* DEVELOPER || ENABLE_SELFTEST */
			...
			const int argc, const char **argv)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_force_election_push(frame, &msg, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}

	ok = send_message(
		msg_ctx, pid, MSG_FORCE_ELECTION, blob.data, blob.length);
out:
	TALLOC_FREE(frame);
	return ok;
}
		       ...
		       const int argc, const char **argv)
{
	...
	frame = talloc_stackframe();
	msg.level = v;
	ndr_err = messaging_profile_push(frame, &msg, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}

	ok = send_message(msg_ctx, pid, MSG_PROFILE, blob.data, blob.length);
out:
	TALLOC_FREE(frame);
	return ok;
}
			    ...
			    DATA_BLOB *data)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		fprintf(stderr,
			"Invalid profilelevel message from PID %u: %s\n",
			      ...
			      DATA_BLOB *data)
{
	...
	send_message(msg_ctx, pid, MSG_PROFILELEVEL, blob.data, blob.length);
out:
	TALLOC_FREE(frame);
}
			    ...
			    const int argc, const char **argv)
{
	...
	ok = send_message(
		msg_ctx, pid, MSG_REQ_PROFILELEVEL, blob.data, blob.length);
	if (!ok) {
		goto out;
	}
			  ...
			  const int argc, const char **argv)
{
	...
	msg.ip = argv[1];
	frame = talloc_stackframe();
	ndr_err = messaging_winbind_ip_dropped_push(frame, &msg, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}

	ok = send_message(
		msg_ctx, pid, MSG_WINBIND_IP_DROPPED, blob.data, blob.length);
out:
	TALLOC_FREE(frame);
	return ok;
}
			 ...
			 const int argc, const char **argv)
{
	...
	messaging_send_iov(
		msg_ctx, dst, MSG_REQ_POOL_USAGE, &iov, 1, &stdout_fd, 1);
	ok = true;
out:
	TALLOC_FREE(frame);
	return ok;
				 ...
				 const int argc, const char **argv)
{
	...
	frame = talloc_stackframe();
	ndr_err = messaging_winbind_disconnect_dc_push(frame, &msg, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
