				   ...
				   DATA_BLOB *data)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		fprintf(stderr,
			"Invalid MSG_DEBUGLEVEL from PID %s: %s\n",
	...
	printf("PID %s: %s",
	       server_id_str_buf(pid, &pidstr),
	       reply.info.info1.debuglevel_string);
	num_replies++;
out:
	TALLOC_FREE(frame);
}
		       ...
		       DATA_BLOB *data)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		DBG_WARNING("Invalid MSG_PONG from pid %s: %s\n",
			    server_id_str_buf(pid, &src_string),
			    ndr_errstr(ndr_err));
		goto out;
	}
		       ...
		       const struct server_id pid)
{
	...
	ndr_err = messaging_ping_push_v1(frame, &msg, "", &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
			       ...
			       DATA_BLOB *data)
{
	...
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		fprintf(stderr,
			"Invalid MSG_PROFILELEVEL from PID %u:"
			" %s\n",
			(unsigned int)procid_to_pid(&pid),
			ndr_errstr(ndr_err));
		goto out;
	}
	level = reply.info.info1.level;
				 ...
				 DATA_BLOB *data)
{
	...
	/* Send back a dummy reply */
	ndr_err = messaging_profilelevel_push_v1(frame, &reply, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
			       ...
			       const struct server_id pid)
{
	...
	/* Send a message and register our interest in a reply */
	ndr_err = messaging_req_profilelevel_push_v1(frame, &msg, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
			     ...
			     const struct server_id pid)
{
	...
	/* Send a message and register our interest in a reply */
	ndr_err = messaging_req_debuglevel_push_v1(frame, &msg, &blob);
	if (!NDR_ERR_CODE_IS_SUCCESS(ndr_err)) {
		goto out;
	}
